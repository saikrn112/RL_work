...
def Startexperiment():
	...
	steps = 0;
	avgsteps = 0;
#	RLGlue.RL_env_message("freeze learning")
	RLGlue.RL_env_message("set-random-start-state");
	for i in range(0,n): # n trials		
#		RLGlue.RL_episode(5000); # 5000 number of steps as a cutoff here in case the policy is bad and will never end an episode
		this_return=RLGlue.RL_return();
		steps+=RLGlue.RL_num_steps();
		sum+=this_return;
	mean=sum/n;
	avgsteps = steps/n;
#	RLGlue.RL_env_message("unfreeze learning")
	return mean,avgsteps;


def saveResultToCSV(statistics, fileName):
	theFile = open(fileName, "w");
	theFile.write("#Returns from each episode\n");
...
RLGlue.RL_agent_message("save_policy results.txt");
